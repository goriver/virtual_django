...
temp, what_day = make_train_data('000100', 20200915)
...
# fig = m.plot(am_pred)
# plt.axis([0, len(am_pred), am_pred['yhat'].min(), am_pred['yhat'].max()])
# plt.savefig('am_predict.png', dpi=400)
fig = go.Figure()
...
# fig = m.plot(pm_pred)
# plt.axis([0, len(pm_pred), pm_pred['yhat'].min(), pm_pred['yhat'].max()])
# plt.savefig('pm_predict.png', dpi=400)
fig = go.Figure()
